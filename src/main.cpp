...
int main(int argc, char** argv)
{
  ...
  try
  {
    ...
    msg_pusher::Pusher *pusher;
    pusher = new msg_pusher::Pusher(io_service, interfaces, addresses, url, id, token, interval, debug);
    ...
    if ( !debug ) {
      ...
      syslog(LOG_INFO | LOG_USER, "msg_pusher started");
    }
    io_service.run();
    if ( !debug )
      syslog(LOG_INFO | LOG_USER, "msg_pusher stopped");
  }
